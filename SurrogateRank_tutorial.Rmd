...
# remotes::install_github("laylaparast/SurrogateRank", ref = "devel")
...
The package also implements the rank-based identification of surrogate markers in the high-dimensional setting. This is described in the paper Hughes A., Parast L., Thiébaut R., Hejblum B.P. (2025). RISE : Two-Stage Rank-Based Identification of High-Dimensional Surrogate Markers Applied to Vaccinology (https://arxiv.org/abs/2502.03030).

Again, let's start by looking at an example dataset from the package. 

```{r}
data(example.data.highdim)
names(example.data.highdim)
```
Like before, this data contains 25 observations from a treatment group (group 1) and 25 observations from a control group (group 0). As in the other example, **y1** and **y0** are the values of the primary response in the treated and control group, respectively. However, this time, **s1** and **s0** are matrices of 1000 candidate surrogate markers in the treated and control group, respectively. The final element of the data, **hyp**, gives whether the surrogate candidate is valid or not, where 'null false' corresponds to a valid surrogate candidate and 'null true' an invalid surrogate. 
...
```
The next element **significant.markers** gives the names of the markers which were judged significant at the provided $\alpha$ level (0.05 by default) after test multiplicity correction. Let's print the first few. 

```{r}
screen.result[["significant.markers"]] %>% head()
```
How many markers were determined significant?
...
```
The third element **gamma.s.evaluate** gives us the results of the testing of gamma as a univariate surrogate in the evaluation data. 

```{r}
evaluate.results[["gamma.s.evaluate"]]
```

The final element, **gamma.s.plot**, gives us a plot of the composite marker gamma.s against the response in the evaluation data. 
```{r}
