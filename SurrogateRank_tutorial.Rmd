...
In this tutorial, we will go through examples using the functions in the **SurrogateRank** package. First, install the package from CRAN or Github and load it.
...
The \code{weight_raw} column gives the pre-normalised weights (i.e. simply the inverse of the delta values). The \code{weight} column divides these weights by the maximum weight, such that the weights now represent a proportion of the maximum value. This is simply for easier interpretation, as using the raw or standardised weights will not change the results of the evaluation. This is because RISE is invariant to **monotonic** (i.e. rank-preserving) transformations. 
